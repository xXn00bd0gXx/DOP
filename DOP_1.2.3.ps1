...
# ─── NEW IN v2.0 ──────────────────────────────────────────────────
$script:LuckTurnsLeft    = 0       # Potion of Luck: temporary +hit%
...
function Check-Achievements {
    $stats = @{
        ...
        quests   = 0  # incremented elsewhere via counter
    ...
    @{ Name="$Type";DisplayName="$Type (Lv$Lvl)";HP=[math]::Floor($b.HP*$s);MaxHP=[math]::Floor($b.HP*$s)
       ...
       Abilities=@(
           ...
           switch($Type){
               ...
               "Skeleton" {@{Name="Bone Throw";Power=9;Type="Physical"}}
        ...
        for($e=0;$e -lt $ne;$e++){
            ...
            if($grid[$ey,$ex]-eq 0){
                ...
                if((Get-Random -Max 100) -lt 5){
                    $enemies["$ex,$ey"]=New-Enemy "Mimic" $Level
                } else {
...
function Render-Screen {
    ...
    if($script:Streak -gt 1){ $hudLines += " Streak: $($script:Streak)x" }
    ...
    for($y=0;$y -lt $vh;$y++){
        ...
        if($y -lt $rightLines.Count){
            ...
            elseif($rl -match "Streak:"){Write-Host $rl -ForegroundColor Yellow -NoNewline}
...
function Start-Combat {
    ...
    # Luck potion adds $LuckBonus to player hit for $LuckTurnsLeft turns.
    function Get-PlayerHit {
        param($level,$spd,$luckBonus)
        $h = 70 + [math]::Min($level - 1, 15) + [math]::Floor($spd * 0.3) + $luckBonus
        if($h -gt 90){ $h = 90 }
        if($h -lt 20){ $h = 20 }
        return $h
    }
    ...
    $playerHitChance = Get-PlayerHit $script:PlayerLevel $p.SPD $script:LuckBonus
    ...
    while($p.HP -gt 0 -and $e.HP -gt 0 -and !$fled){
        ...
        $luckStr = if($script:LuckTurnsLeft -gt 0){" | LUCK +$($script:LuckBonus)%  ($($script:LuckTurnsLeft)t)"}else{""}
        Write-CL "  Hit:$($playerHitChance)%$luckStr | Enemy Hit:$($enemyHitChance)%" "DarkGray"
        ...
        Write-CL "   [1] Attack  [2] Ability  [3] Potion  [4] Throw($throwCount)  [5] Defend  [6] Flee" "White"
        Write-CL ("  " + "-" * 66) "DarkGray"
        Write-C "  > " "Yellow"
        $choice = Read-Host
        ...
        switch($choice){
                ...
                if($script:Potions.Count -eq 0){
                    ...
                    if($pidx -ge 0 -and $pidx -lt $script:Potions.Count){
                        ...
                        switch($pot.Type){
                            ...
                            "Luck" {
                                # New: Luck potion boosts hit chance for several turns
                                $script:LuckBonus = $pot.Power
                                $script:LuckTurnsLeft = 5
                                $playerHitChance = Get-PlayerHit $script:PlayerLevel $p.SPD $script:LuckBonus
                                [void]$combatLog.Add(@{Text=">> LUCK +$($pot.Power)% hit for 5 turns!";Color="Yellow"})
                            }
                        }
            ...
            "5" {
                # NEW: Defend. Halves incoming damage this turn. Trade-off: no damage dealt.
        ...
        if($e.HP -gt 0 -and !$fled -and $acted){
            ...
            if($e.Stunned){
                ...
                # Enemy miss check (new percent-based system)
                if((Get-Random -Max 100) -ge $enemyHitChance){
                    ...
                    # Defending DOUBLES the DEF this turn
                    if($playerDefending){ $totalPlayerDEF = $totalPlayerDEF * 2 }

                    $eAbility = if($forcedBossAbility){$forcedBossAbility}else{$e.Abilities | Get-Random}
        ...
        if($script:LuckTurnsLeft -gt 0 -and $acted){
            ...
            if($script:LuckTurnsLeft -le 0){
                $script:LuckBonus = 0
                $playerHitChance = Get-PlayerHit $script:PlayerLevel $p.SPD 0
    ...
    # ── Lockpick drops: regular enemies only (not boss, miniboss), 35% chance, 1-5 picks ──
    if(-not $e.IsBoss -and -not $e.IsMiniBoss){
        if((Get-Random -Max 100) -lt 35){
            $picksDropped = Get-Random -Min 1 -Max 6  # 1..5 inclusive
            $script:Lockpicks += $picksDropped
            $word = if($picksDropped -eq 1){"lockpick"}else{"lockpicks"}
            Write-CL "  + $picksDropped $word found on the body" "Cyan"
        }
    }
    if($e.DropsKey){
                ...
                while($wepLoop){
                    ...
                    if($widx -ge 0 -and $widx -lt $filtered.Count){
                        ...
                        if($script:Gold -ge $w.Price){
                            ...
                            $script:EquippedWeapon = Init-ItemDurability $w
                ...
                while($armorLoop){
                    ...
                    if($aIdx -ge 0 -and $aIdx -lt $slotArmor.Count){
                        ...
                        if($script:Gold -ge $a.Price){
                            ...
                            $script:EquippedArmor[$chosenSlot] = Init-ItemDurability $a
            ...
            "3" {
                ...
                while($potLoop){
                    ...
                    $potionShop = @(
                        ...
                        @{Name="Potion of Luck";      Type="Luck";    Power=20; Price=90;  Desc="+20% hit, 5 turns"; Icon="[LCK]";  Category="Potion"}
...
function Start-RandomEncounter {
    ...
    $p = $script:Player
    if(-not $Type){
        # Pick from available options. Alchemist can't spawn if potion bag is full
        # (the trade would be impossible / wasted).
        $options = @("Bard", "Merchant")
    ...
    switch($Type){
                ...
                if($script:Gold -ge $salePrice){
                    ...
                    switch($itemKind){
                        "Weapon" {
                            ...
                            $script:EquippedWeapon = Init-ItemDurability $item
                            $script:WeaponsOwned[$item.Name] = $true
                            Write-CL "  Equipped $($item.Name)!" "Green"
                        }
                        "Armor" {
                            $script:EquippedArmor[$item.Slot] = Init-ItemDurability $item
        ...
        "Dutchman" {
            ...
            if($call -eq $result){
                ...
                $script:EquippedWeapon = Init-ItemDurability $dutchBlade
...
function Start-Lockpicking {
    ...
    # Rhythm — a bit faster as more tumblers
    ...
    function lp-TestSweet([int]$i){ return ((lp-GetPinRow $i) -lt $SZ) }
    ...
    $renderFrame = {
        ...
        for($r=0;$r -lt $GH;$r++){
            ...
            if($r -eq $SZ){
                ...
                $divLine += $V
                Write-Host (lp-Pad $divLine) -ForegroundColor DarkGray
            }
            Write-Host "$lp$V" -NoNewline -ForegroundColor Cyan
            for($c=0;$c -lt $TC;$c++){
                if($c -gt 0){
        ...
        # Timing bar
        $sync = -[Math]::Sin($script:lpT * $script:lpFrq[$script:lpCur] + $script:lpPhs[$script:lpCur])
        $barLen = 20
...
function Enter-Dungeon {
    ...
    while($inDungeon -and $script:Player.HP -gt 0){
        ...
        # All keys register instantly — no Enter needed.
        ...
        switch($key.ToUpper()){
                ...
                if($nx -ge 0 -and $nx -lt $d.W -and $ny -ge 0 -and $ny -lt $d.H -and $d.Grid[$ny,$nx] -ne 1){
                    ...
                    elseif($cell -eq 6){
                        ...
                        elseif($script:Lockpicks -lt 1){
                            ...
                            $tumblers = 3
                            if($script:DungeonLevel -ge 4){ $tumblers = 4 }
                            if($script:DungeonLevel -ge 8){ $tumblers = 5 }
                            ...
                            if($result.Success){
                                ...
                                Write-Host ""
                                $loot = New-RandomLoot ($script:DungeonLevel + $tumblers - 3)
                                [void]$script:Inventory.Add($loot)
                                # Scale gold bonus with tumbler difficulty
                                # Gold reward scales with tumblers + dungeon level. Always use a range
                                # where min < max to avoid Get-Random min-equals-max errors.
                                $goldMin = 10 * $tumblers
                                $goldMax = $goldMin + (20 * $tumblers) + (15 * $script:DungeonLevel)
                                $tGold = Get-Random -Min $goldMin -Max $goldMax
                                $tGold = [int]$tGold
                                $script:Gold += $tGold
                                Write-CL "  + $($loot.Name) (Value: $($loot.Value)g)" "Magenta"
                                Write-CL "  + ${tGold}g from inside the chest" "Yellow"

                                # Small chance of rare drop: potion or bonus lockpicks
                                ...
                                if($rareRoll -lt 10){
                                    ...
                                    if($script:Potions.Count -lt 10){
                                        [void]$script:Potions.Add($potDrop)
                                        Write-CL "  + A hidden vial: $($potDrop.Name)" "Green"
                                    } else {
                                        Write-CL "  (A potion was inside, but your satchel is full.)" "DarkGray"
                                    }
                                } elseif($rareRoll -lt 25){
                                    # 15% bonus lockpicks
                                    $bonusPicks = Get-Random -Min 1 -Max 4
                                    $script:Lockpicks += $bonusPicks
                                    Write-CL "  + $bonusPicks bonus lockpick(s) tucked inside!" "Cyan"
                                }

                                Update-QuestProgress "Treasure"
                                $d.Grid[$ny,$nx] = 0      # remove chest
                                $d.PX = $nx; $d.PY = $ny
                                Write-Host ""
                                Read-Host "  [Press Enter to continue]" | Out-Null
                    ...
                    elseif($cell -eq 7){
                        ...
                        # Dutchman: level 5+, daily-dungeon only, not if already won.
                        # Non-Dutchman encounters: ~1.5% per step.
                        $roll = Get-Random -Minimum 0 -Maximum 1000
                        $dutchAllowed = (
                            $script:DailyDungeonActive -and
                            $script:PlayerLevel -ge 5 -and
            ...
            "S" {
                ...
                if($nx -ge 0 -and $nx -lt $d.W -and $ny -ge 0 -and $ny -lt $d.H -and $d.Grid[$ny,$nx] -ne 1){
                    $d.PX = $nx; $d.PY = $ny
                    # Backward step: slightly rarer ~0.8%
                    $roll = Get-Random -Minimum 0 -Maximum 1000
                    $dutchAllowed = (
                        $script:DailyDungeonActive -and
                        $script:PlayerLevel -ge 5 -and
...
function Get-ClassTemplates {
    @{
        ...
        Berserker = @{
            ...
            Abilities=@(
                @{Name="Blood Rage";   Cost=6;  Type="Buff";     Power=0;  Effect="ATK8"}
                @{Name="Cleave";       Cost=10; Type="Physical"; Power=22; Effect="Bleed"}
                @{Name="War Cry";      Cost=8;  Type="Physical"; Power=8;  Effect="Stun"}
                @{Name="Undying Fury"; Cost=0;  Type="Sacrifice";Power=40; Effect="SacrificeHP"}
            )
        ...
        Warlock = @{
            ...
            Abilities=@(
                @{Name="Eldritch Blast"; Cost=10; Type="Magic";     Power=24; Effect="None"}
                @{Name="Hex";            Cost=12; Type="Magic";     Power=15; Effect="Weaken"}
                @{Name="Summon Imp";     Cost=18; Type="Magic";     Power=20; Effect="Burn"}
                @{Name="Dark Bargain";   Cost=0;  Type="Sacrifice"; Power=38; Effect="SacrificeHP"}
            )
...
if(-not $script:AvailableQuests -or $script:AvailableQuests.Count -eq 0){
    ...
    while($qbLoop){
        ...
        $optRows = @(
            ...
            @{N="[T]"; C="Yellow";  L="Turn in completed quest"}
...
function Get-PotionShop {
    @(
        ...
        @{Name="Potion of Luck";      Type="Luck";        Power=20; Price=90; Desc="+20% hit, 5 turns"; Category="Potion"}
...
function Load-Game {
    ...
    $parts = $saveStr -split '\|'
    $ver = $parts[0]
    # Only v3 accepted. v1 and v2 were pre-durability and don't track per-item HP,
    # so loading them would leave the player with no-durability gear that can never
    # be repaired. Clean reject with explanation.
    if($ver -eq "1" -or $ver -eq "2"){
        return @{ Success = $false; Error = "This save is from an older version and can't be loaded. The durability update requires a new character." }
    }
    if($ver -ne "3"){
        return @{ Success = $false; Error = "Unknown save format version '$ver'." }
    }
    # v3: 35 parts (34 data + 1 checksum)
    $expectedLen = 35
    if($parts.Count -ne $expectedLen){
        return @{ Success = $false; Error = "Save code is corrupted (wrong length for v3)." }
    }
...
function Show-MainMenu {
    while($script:GameRunning){
        ...
        $lv = "Lv$($script:DungeonLevel + 1)"
        $leftCell  = " [1] Enter Dungeon  $lv"
        ...
        if($rPad -lt 0){$rPad=0}
        Write-C "  ║" "DarkGray"
        Write-C " [1]" "Green"; Write-C " Enter Dungeon  " "White"; Write-C $lv "DarkYellow"
        Write-C ("$(' ' * $lPad)║") "DarkGray"
        ...
        switch($routed){
            ...
            "2" { Show-Market }
                ...
                while($invLoop){
                    ...
                    $potHdr = " P O T I O N S  ($($script:Potions.Count) / 10)"
                    Write-C "  ║" "DarkGreen"
                    Write-C (Pad-Box $potHdr) "Green"
                    Write-CL "║" "DarkGreen"
                    Write-CL ("  ╠" + $barH + "╣") "DarkGreen"
                    if($script:Potions.Count -eq 0){
                        Write-C "  ║" "DarkGreen"
                        ...
                        foreach($key in $potionGroups.Keys){
                            ...
                            $typeIcon = switch($pg.Type){
                                "Heal"{"[HP]"} "Mana"{"[MP]"} "ATKBuff"{"[AT]"} "DEFBuff"{"[DF]"} "Luck"{"[LK]"} default{"[??]"}
                            }
                            $typeColor = switch($pg.Type){
                                "Heal"{"Green"} "Mana"{"Cyan"} "ATKBuff"{"Yellow"} "DEFBuff"{"Yellow"} "Luck"{"Magenta"} default{"White"}
                            }
                            $countStr = " x$($pg.Count)"
                            Write-C "  ║" "DarkGreen"
                            Write-C " $typeIcon" $typeColor
                            Write-C " $key" "White"
                            Write-C $countStr "DarkGray"
                            $used = 1 + $typeIcon.Length + 1 + $key.Length + $countStr.Length
                            $pad = $BoxW - $used
                            if($pad -lt 0){$pad = 0}
                            Write-C (" " * $pad) "Black"
                            Write-CL "║" "DarkGreen"
                    ...
                    # ── Throwables ──
                    Write-CL ("  ╔" + $barH + "╗") "DarkYellow"
                    $thrHdr = " T H R O W A B L E S  ($($script:ThrowablePotions.Count) / 5)"
                    Write-C "  ║" "DarkYellow"
                    Write-C (Pad-Box $thrHdr) "DarkYellow"
                    Write-CL "║" "DarkYellow"
                    Write-CL ("  ╠" + $barH + "╣") "DarkYellow"
                    if($script:ThrowablePotions.Count -eq 0){
                        Write-C "  ║" "DarkYellow"
                        ...
                        foreach($key in $throwGroups.Keys){
                            ...
                            Write-C " $key" "White"
                            Write-C $countStr "DarkGray"
                            $used = 1 + $tIcon.Length + 1 + $key.Length + $countStr.Length
                            $pad = $BoxW - $used
                            if($pad -lt 0){$pad = 0}
                            Write-C (" " * $pad) "Black"
                            Write-CL "║" "DarkYellow"
                    ...
                    if($script:Inventory.Count -eq 0){
                        ...
                        Write-CL ("  ╠" + $barH + "╣") "DarkMagenta"
                        Write-C "  ║" "DarkMagenta"
                        Write-C " Total Sell Value: " "White"
                        Write-C "${totalVal}g" "Yellow"
                        $used = 1 + 18 + "${totalVal}g".Length
                        $pad = $BoxW - $used
                        if($pad -lt 0){$pad = 0}
                        Write-C (" " * $pad) "Black"
                        Write-CL "║" "DarkMagenta"
                    ...
                    $oBar = "─" * 44
                    Write-CL "  ┌$oBar┐" "DarkGray"
                    $row1 = " [U] Unequip Gear (20% refund)"
                    $pad1 = 42 - $row1.Length
                    if($pad1 -lt 0){$pad1 = 0}
                    Write-C "  │" "DarkGray"; Write-C " [U]" "Red"; Write-C " Unequip Gear (20% refund)" "White"
                    Write-CL ("$(' ' * $pad1) │") "DarkGray"
                    $row2 = " [0] Back"
                    $pad2 = 42 - $row2.Length
                    if($pad2 -lt 0){$pad2 = 0}
...
function Show-Changelog {
    $pages = @(
        @{Title="NEW FEATURES"; Color="Green"; Lines=@(
            "  CHARACTER & CLASSES"
            "    * Two new classes: Berserker (sword) and Warlock (staff)"
            "    * Shared weapon affinity: Berserker gets sword bonuses,"
            "      Warlock gets staff bonuses"
            "    * Dedicated weapon lines for both new classes in the shop"
            ""
            "  DUNGEON & EXPLORATION"
            "    * Full 3D first-person dungeon rendering (56x24 viewport)"
            "      with half-block wall textures, depth fog, and torches"
            "    * 10-wide detailed ASCII sprites for every enemy type"
            "      (Goblin, Zombie, Thief, Wizard, Troll, Skeleton, etc.)"
            "    * Class-specific sprites for all 8 classes (used by Mimic"
            "      enemy, which copies your class on encounter)"
            "    * Real-time input for all dungeon keys — no Enter needed"
            ""
            "  COMBAT"
            "    * Hit-chance system: 70% base player, 65% enemy,"
            "      capped 20-90%"
            "    * Defend action, boss telegraph for devastating attacks"
            "    * Per-weapon-type combat animations (sword, staff, fist, bow)"
            "    * Enemy death dissolve animation"
            "    * Potion of Luck (+20% hit chance for 5 turns)"
            ""
            "  NEW GAME MODES & SYSTEMS"
            "    * Daily Dungeon — 2x rewards, one attempt per calendar day"
            "    * Training Grounds — spend training points on stats"
            "    * 22 Achievements with gold/XP rewards"
            "    * Quest Board with active quests and rewards"
            "    * Guild Hall + three partners (Healer, Thief, Bard)"
            "    * Weary Lantern inn with Quick/Full/Royal rest tiers"
            ""
            "  RANDOM ENCOUNTERS"
            "    * Healing Bard — restores HP/MP"
            "    * Lost Merchant — sells items at 60% off"
            "    * Wandering Alchemist — trades loot for potions"
            "    * Flying Dutchman (rare, level 5+, Daily only)"
            "      coin-toss wager for legendary blade or level loss"
            ""
            "  ENDGAME CONTENT"
            "    * Endgame weapons (ATK 25-40) across all weapon types"
            "    * Legendary armor (DEF 15-24) in all 5 slots"
            "    * Dutchman's Blade: ATK+100, Drain 40%, indestructible"
            ""
            "  LATEST ADDITIONS (this version)"
            "    * Lockpicking minigame for all treasure chests"
            "    * Lockpicks as a separate inventory item (start with 5)"
            "    * Locksmith stall in the market (25g per pick)"
            "    * Full weapon/armor durability system"
            "    * Blacksmith stall for gear repair"
        )}
        @{Title="GAMEPLAY IMPROVEMENTS"; Color="Cyan"; Lines=@(
            "  PROGRESSION"
            "    * Clear-streak gold bonus: +15% per streak, up to 90%"
            "    * Best-ever streak persists across saves"
            "    * Current session streak shown on stats page"
            "    * Boss difficulty scaling softened"
            "      (HP x0.35, ATK x0.32 per level)"
            ""
            "  LOOT & REWARDS"
            "    * Chest loot scales with lockpicking difficulty"
            "    * Lockpicks drop from regular enemies (35% chance, 1-5)"
            "    * 10% chance of bonus potion inside picked chests"
            "    * 15% chance of bonus lockpicks inside picked chests"
            ""
            "  INPUT & CONTROLS"
            "    * Real-time WASD movement in dungeon (hold for repeat)"
            "    * Arrow keys mapped to WASD"
            "    * P for potion, Q for quit — all instant"
            "    * Key buffer drained between calls to prevent runaway"
            ""
            "  ENCOUNTER BALANCE"
            "    * Non-Dutchman encounter rates reduced (1.5% fwd / 0.8% back)"
            "    * Alchemist won't spawn if potions are full"
            ""
            "  UI & DISPLAY"
            "    * Inventory screen shows durability per equipped item"
            "    * Stats page shows current streak, best streak, bosses"
            "    * Save code auto-copied to clipboard on save"
            "    * Tutorial expanded to cover durability and save caveats"
            ""
            "  MARKET POLISH"
            "    * Fantasy market ASCII art (Old Quarter Market banner)"
            "    * Town skyline, Flying Dutchman ghost ship, forge art"
            "    * 60-character-wide consistent shop banners"
        )}
        @{Title="BUG FIXES & QUALITY OF LIFE"; Color="Yellow"; Lines=@(
            "  RENDERING"
            "    * Treasure chest sprite no longer shows wall-outline bleed"
            "      through its negative space"
            "    * Weapon/Armor/Potion shop tables now use column-width"
            "      discipline — two-digit rows render identically to single"
            "    * Inn 'Weary Lantern' art restored to original reference"
            ""
            "  INPUT"
            "    * Held movement key no longer causes endless walking"
            "    * Encounters with no user input (Bard etc.) now require"
            "      Enter to dismiss — held keys cannot skip them"
            "    * 150ms pause on encounter entry for deliberate feel"
            ""
            "  CRASHES FIXED"
            "    * Inventory page crash (System.Collections.Hashtable"
            "      comparison error). Root cause: PowerShell variable names"
            "      are case-insensitive, so `$W` (column width) collided"
            "      with `$w` (equipped weapon). Renamed to `$BoxW`."
            "    * Get-Random crash after lockpicking when min == max on"
            "      low dungeon levels. Gold range now always has min < max."
            ""
            "  TEXT & LABELS"
            "    * Duplicate class names removed from dungeon HUD"
            "      (was 'Berserker Lv2 (Berserker)', now just 'Berserker Lv2')"
            "    * Duplicate class names removed from stats page header"
            "    * Equipment details removed from stats page (now only"
            "      in inventory — eliminates redundancy)"
            ""
            "  TUTORIAL"
            "    * New section on gear durability mechanics"
            "    * Note explaining held-key trade-off for real-time input"
            "    * Clarifies that current streak does NOT persist across"
            "      saves, but best-ever streak does"
            "    * Berserker and Warlock added to affinity list"
            ""
            "  SAVE SYSTEM"
            "    * Save format bumped to v3 (durability-aware)"
            "    * Old v1 and v2 saves rejected cleanly with explanatory"
            "      message rather than silent failure"
            "    * New-character requirement clearly communicated"
            ""
            "  REMOVED CLUTTER"
            "    * Launch screen: Controls and Symbols legend boxes"
            "      removed (tutorial covers it)"
        )}
    )
    ...
    foreach($page in $pages){
        ...
        Write-Host ""
        Write-CL "  ╔══════════════════════════════════════════════════════════════════╗" "DarkMagenta"
        Write-CL "  ║                    C H A N G E L O G   -   v1.2.3                ║" "Magenta"
        Write-CL "  ╚══════════════════════════════════════════════════════════════════╝" "DarkMagenta"
        Write-Host ""
...
function Start-Game {
    ...
    Write-Host ""
    Write-CL "                  Version 1.2.3" "DarkGray"
    Write-Host ""
...
function Show-Tutorial {
    $pages = @(
        @{Title="1. THE TOWN SQUARE"; Lines=@(
            ...
            "   * Visit the bazaar to buy gear, armor, potions"
            "   * Rest at the Weary Lantern Inn to recover HP/MP"
            "   * Train at the Training Grounds (boost stats for gold)"
            "   * Accept quests for bonus rewards"
            "   * Hire companions from the Guild Hall"
            ""
            "  You select actions by typing their number or letter."
        )}
        @{Title="2. DUNGEON MOVEMENT"; Lines=@(
            "  Inside a dungeon, movement is REAL-TIME. Just press keys — no Enter needed."
            ""
            "  Controls:"
            "    W — walk forward         S — step back"
            "    A — turn left            D — turn right"
            "    P — use a healing potion Q — quit the dungeon"
            "    (arrow keys also work)"
            ""
            "  NOTE: Sometimes you may need to hold a key down briefly for it"
            "  to register. This is a small trade-off for real-time input in"
            "  a PowerShell console — console input isn't quite as responsive"
            "  as a dedicated game engine."
            ""
        ...
        @{Title="3. COMBAT BASICS"; Lines=@(
            ...
            "    [2] Ability  — class spells/skills (costs MP)"
            "    [3] Potion   — drink from your potion bag"
            "    [4] Throw    — hurl flasks and bombs"
            "    [5] Defend   — double DEF this turn (useful vs. boss charges!)"
        ...
        @{Title="4. GEAR & DURABILITY"; Lines=@(
            ...
            "   * Stronger gear has more durability but also costs more to fix."
            ""
        ...
        @{Title="5. SAVING YOUR PROGRESS"; Lines=@(
